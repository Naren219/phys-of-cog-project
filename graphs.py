...
def initialize_epochs():
    ...
    return conds, results, colordict, CH_TO_GROUP, clim


LAGS_SEC = np.linspace(-0.05, 0.05, 81)

conds, results, colordict, CH_TO_GROUP, clim = initialize_epochs()

def run_graph(condition_name, threshold, show_plots = False):
    ep = conds[condition_name]
    ...
    def build_corr_graph(epochs, mask, weights, threshold, use_abs=True):
    ...
    epfjq, wepof, global_corr, opt_lag, lag_applied = redraw(condition_name)
    ep_avg = np.mean(ep.get_data(), axis=0)
    ...
    for i in range(len(opt_lag)):
        ...
        for j in range(len(opt_lag)):
            ...
            lagged_graph = lagged_corr_2d(ep_avg, lag_samples)
    ...
    if show_plots:
        ...
        plt.show()
    initialize_graph(ep)
    g_signal_noise = build_corr_graph(ep,mask = signal_noise_ratio,weights = new_corr, threshold = threshold)
    #plt.figure()
    #fig, ax = plt.subplots()
    #ax = nx.draw(g_signal_noise, pos = nx.forceatlas2_layout(g_signal_noise),node_color = [g_signal_noise.nodes[n]['color'] for n in g_signal_noise.nodes], with_labels = True, ax = ax)
    #fig.show()
    plot_3d_graph(g_signal_noise)
    return g_signal_noise

run_graph("BLT", threshold = .5,show_plots = False)
run_graph("BLA", threshold = .5)
